...
describe("Book Property", function () {
	...
	it("Book Property but for tokenId does not exist", async function () {
		const tokenHolderAddress = await tokenHolder.getAddress();
		...
		const tx = marketplaceContract.connect(marketplace).bookProperty(2, false);
		await expect(tx).to.be.reverted;
	});

	it("Book Property with existing tokenId", async function () {
		const tokenHolderAddress = await tokenHolder.getAddress();
