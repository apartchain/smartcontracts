...
describe("Cancel Booking", function () {
	...
	it("Cancel Booking for tokenId does not exist", async function () {
		...
		const tx = marketplaceContract.connect(marketplace).cancelBooking(2, false);
	...
	it("Cancel Booking by other than marketplace role", async function () {
		...
		await marketplaceContract.connect(buyer).bookProperty(1, false);
		const tx = marketplaceContract.connect(owner).cancelBooking(1, false);
	...
	it("Successful Cancel Booking", async function () {
		...
		const initialBalanceBuyer = await usdcContract.balanceOf(buyerAddress);
		...
		await marketplaceContract.connect(buyer).bookProperty(1, false);
		const tx = marketplaceContract.connect(marketplace).cancelBooking(1, true);
		await expect(tx).not.to.be.reverted;

		const finalBalanceBuyer = await usdcContract.balanceOf(buyerAddress);

		expect(finalBalanceBuyer).to.equal(initialBalanceBuyer);
